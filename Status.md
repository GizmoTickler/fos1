...
This repository has progressed from an architectural blueprint to a **functional implementation** with Sprint 31 (tickets 47-55) fully merged as of 2026-04-25, on top of Sprint 30 (tickets 38-46) and Sprint 29 (tickets 29-37). Sprint 32 has started with Ticket 56 from Linear. `make verify-mainline` remains the canonical gate. The primary routing, NAT, DNS, DHCP, NTP, WireGuard, IDS, DPI, auth, policy-enforcement, eBPF (XDP + TC + sockops + cgroup), QoS (per-pod egress + VLAN-scoped TC shaper), threat-intel (URLhaus + MISP), CRUD-v1 REST API, controller HA, internal TLS, mTLS mesh primitives, and observability-proof pipelines are implemented or in active hardening.
...
| Production Ready | ❌ NO (estimated ~83-88%) | Ticket 56 narrows the mTLS mesh gap, but residual blockers remain: Prometheus client-cert scrape rekey, external-daemon TLS/HA, shared-state HA, and API breadth |

## Priority Next Steps

Sprint 31 (tickets 47-55) is fully merged. Sprint 32 is active, starting with Ticket 56.

Candidate Sprint 32 workstreams (in rough priority order):

1. **Prometheus client-cert scrape rekey** — Ticket 56 moved the current owned HTTP listener mesh to mTLS, so Prometheus now needs `tls_config.ca_file`, `cert_file`, and `key_file` for the `prometheus` client identity before live scrapes of mTLS targets can pass.
...
3. **External-daemon HA** — Ticket 47 covered controller-tier failover only. FRR / Suricata / Zeek / Kea singletons run as single-pod / single-process; per-daemon clustering (BFD for FRR, Kea HA hooks, parallel Suricata sensors) remains open.
...
7. **Broader eBPF program types** — Ticket 51 added sockops + cgroup; sk_msg, sk_lookup, lwt, and other program types still return `ErrEBPFProgramTypeUnsupported` from `pkg/hardware/ebpf/program_manager.go`.
...
| **Internal TLS / mTLS Mesh** | `pkg/security/certificates/`, `manifests/base/certificates/cluster-issuer-internal.yaml` | - | In progress (server TLS complete, mTLS mesh started) | Sprint 31 Ticket 49 delivered hot-reloaded server TLS from `fos1-internal-ca`. Sprint 32 Ticket 56 adds `LoadMutualTLSConfig`, `NewMutualTLSHTTPClient`, and Subject-CN allowlist middleware; currently owned non-API listeners now enforce mTLS when TLS is enabled. `scripts/ci/prove-cert-rotation.sh` covers rotation and `scripts/ci/prove-mtls-mesh.sh` covers allowed cert / no cert / unknown CN behavior. External-daemon TLS and Prometheus client-cert scrape rekey remain Sprint 32 follow-ups. See `docs/design/internal-tls-secrets.md`. |
...
- Prometheus scrape configs in `manifests/base/monitoring/prometheus.yaml` still need `tls_config.ca_file`, `cert_file`, and `key_file` for the `prometheus` client identity. mTLS targets fail closed until Ticket 57 lands.
...
1. **Prometheus Rekey for `fos1-internal-ca`** - Scrape configs need CA trust plus a `prometheus` client certificate (`ca_file`, `cert_file`, `key_file`) before mTLS metrics targets scrape successfully.
2. **External-Daemon TLS** - FRR vtysh, Suricata socket, Kea control socket, Zeek Broker, and chronyc still speak plaintext on in-pod loopback / Unix paths. Cross-host paths need a sidecar TLS terminator or daemon-native TLS.
...
- mTLS mesh primitives are now in progress via Ticket 56; Prometheus still needs client-cert scrape rekey for the `fos1-internal-ca` chain.
- External-daemon TLS still open for FRR, Suricata, Kea, Zeek, and chronyc.
