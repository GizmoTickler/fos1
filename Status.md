...
| Production Ready | ❌ NO (estimated ~83-88%) | Tickets 56-57 narrow the mTLS mesh and observability scrape gaps, but residual blockers remain: external-daemon TLS/HA, shared-state HA, and API breadth |
...
1. **External-daemon TLS** — Tickets 58-62 still need to move FRR, Suricata, Kea, Zeek, and chrony control paths off plaintext loopback/Unix socket assumptions.
...
17. ~~No Inter-Controller TLS Baseline~~ - **Resolved (baseline):** Sprint 31 Ticket 49 ships the `fos1-internal-ca` ClusterIssuer + cert-manager rotation reload. Ticket 56 adds owned-listener mTLS, and Ticket 57 adds the Prometheus client-cert scrape rekey. External-daemon TLS remains a Sprint 32 follow-up.
...
- mTLS mesh primitives are now in progress via Ticket 56, and Prometheus client-cert scrape rekey is implemented locally via Ticket 57.
