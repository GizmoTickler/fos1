...
This repository has progressed from an architectural blueprint to a **functional implementation** with Sprint 31 (tickets 47-55) fully merged as of 2026-04-25, on top of Sprint 30 (tickets 38-46) and Sprint 29 (tickets 29-37). Sprint 32 has started with Tickets 56-59 from Linear. `make verify-mainline` remains the canonical gate. The primary routing, NAT, DNS, DHCP, NTP, WireGuard, IDS, DPI, auth, policy-enforcement, eBPF (XDP + TC + sockops + cgroup), QoS (per-pod egress + VLAN-scoped TC shaper), threat-intel (URLhaus + MISP), CRUD-v1 REST API, controller HA, internal TLS, mTLS mesh primitives, FRR vtysh mTLS sidecar, Suricata command auth/TLS sidecar, and observability-proof pipelines are implemented or in active hardening.
...
| Production Ready | ❌ NO (estimated ~85-90%) | Tickets 56-59 narrow the mTLS mesh, observability scrape, FRR daemon-control TLS, and Suricata command TLS gaps, but residual blockers remain: external-daemon TLS/HA, shared-state HA, and API breadth |

## Priority Next Steps

Sprint 31 (tickets 47-55) is fully merged. Sprint 32 is active, with Tickets 56-59 underway.

Candidate Sprint 32 workstreams (in rough priority order):

1. **External-daemon TLS** — Ticket 58 moves FRR vtysh behind a sidecar mTLS terminator, and Ticket 59 moves Suricata command traffic behind a repo-owned mTLS/shared-secret sidecar. Tickets 60-62 still need to move Kea, Zeek, and chrony control paths off plaintext loopback/Unix socket assumptions.
...
**Still Missing:**
- FRR vtysh and Suricata command traffic now have repo-owned mTLS sidecars. Zeek Broker, Kea's control socket, and chronyc still live inside the same pod as their controller and speak plaintext on a loopback / Unix path. Cross-host paths are the Sprint 32 follow-up — likely sidecar TLS terminators or daemon-native TLS.
...
2. **External-Daemon TLS** - FRR vtysh and Suricata command traffic are implemented locally via sidecar mTLS terminators. Kea control socket, Zeek Broker, and chronyc still speak plaintext on in-pod loopback / Unix paths. Cross-host paths need a sidecar TLS terminator or daemon-native TLS.
...
17. ~~No Inter-Controller TLS Baseline~~ - **Resolved (baseline):** Sprint 31 Ticket 49 ships the `fos1-internal-ca` ClusterIssuer + cert-manager rotation reload. Ticket 56 adds owned-listener mTLS, Ticket 57 adds the Prometheus client-cert scrape rekey, Ticket 58 adds the FRR vtysh mTLS sidecar, and Ticket 59 adds the Suricata command mTLS/shared-secret sidecar. External-daemon TLS for Kea / Zeek / chronyc remains a Sprint 32 follow-up.
...
23. **mTLS Controller-To-Controller + External-Daemon TLS** - Ticket 49 shipped server-side TLS only; Tickets 56-59 narrow the gap with owned-listener mTLS, Prometheus client certs, FRR vtysh mTLS, and Suricata command mTLS/shared-secret auth. TLS for Kea / Zeek / chronyc remains open.
...
- mTLS mesh primitives are now in progress via Ticket 56, Prometheus client-cert scrape rekey is implemented locally via Ticket 57, FRR vtysh mTLS sidecar is implemented locally via Ticket 58, and Suricata command mTLS/shared-secret auth is implemented locally via Ticket 59.
- External-daemon TLS still open for Kea, Zeek, and chronyc.
