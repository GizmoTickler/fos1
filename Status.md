...
- **URLhaus threat-intel v0** — `ThreatFeed` CRD + `cmd/threatintel-controller/` + `pkg/security/threatintel/` parses URLhaus CSV and translates into Cilium deny policies with last-seen TTL. `ThreatFeed.Status` reports last-fetch time, entry count, expiry state. STIX remains a non-goal; MISP JSON ingestion followed in Sprint 31 Ticket 53 (API-key via `spec.authSecretRef` → Secret `apiKey` data key) (Ticket 44 + Ticket 53).
