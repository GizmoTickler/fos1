...
This repository has progressed from an architectural blueprint to a **functional implementation** with Sprint 31 (tickets 47-55) fully merged as of 2026-04-25, on top of Sprint 30 (tickets 38-46) and Sprint 29 (tickets 29-37). `make verify-mainline` is green (`go test ./...` 43/43 packages pass, `go build ./...` succeeds). The primary routing, NAT, DNS, DHCP, NTP, WireGuard, IDS, DPI, auth, policy-enforcement, eBPF (XDP + TC + sockops + cgroup), QoS (per-pod egress + VLAN-scoped TC shaper), threat-intel (URLhaus + MISP), CRUD-v1 REST API, controller HA, internal TLS, and observability-proof pipelines are implemented.
...
- **Inter-controller TLS baseline (Ticket 49)** — single `fos1-internal-ca` ClusterIssuer (CA-typed, chained from a 10y self-signed root) mints per-controller server certs at `/var/run/secrets/fos1.io/tls/`. The shared `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` helper reloads cert-manager renewals in place via fsnotify — no pod restart, no listener bounce. API server stays mTLS-only; other owned listeners (NTP exporter, DPI metrics, correlator probes) serve TLS-only with `ClientAuth = NoClientCert`. `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew`. mTLS for controller-to-controller calls and TLS for external daemons (FRR / Suricata / Kea / Zeek / chronyc) deferred to Sprint 32. See `docs/design/internal-tls-secrets.md`.
...
What remains for production readiness: external-daemon HA (FRR / Suricata / Kea / Zeek singletons); shared-state HA (Elasticsearch / Prometheus / Grafana / Alertmanager); mTLS controller-to-controller and external-daemon TLS (Ticket 49 left this for Sprint 32); Prometheus rekey for the `fos1-internal-ca` chain; write-path API for additional resource families (NAT, routing, DPI, zones); watch / streaming endpoints; broader eBPF program types beyond the four owned loaders (sk_msg, sk_lookup, etc.). See `docs/design/implementation_backlog.md` §"Sprint 32 (placeholder): Post-Sprint-31 Production Hardening".
...
| Primary Ticket Track | Tickets 1-55 complete (Sprints 29, 30, and 31 all fully merged) | Core path + observability proof depth + FilterPolicy enforcement + auth closeout + NIC/capture reporting + coverage bumps + eBPF compile+load (XDP+TC+sockops+cgroup) + shared status writeback helper + CRUD v1 REST API + RBAC baseline + four-hot-path perf baseline + URLhaus + MISP threat-intel + per-pod egress QoS + VLAN-scoped TC shaper + controller leader election + inter-controller TLS baseline |
| Remaining Work Shape | Sprint 32 (placeholder) post-Sprint-31 production hardening | mTLS controller-to-controller; external-daemon TLS (FRR / Suricata / Kea / Zeek); Prometheus rekey for `fos1-internal-ca`; external-daemon HA; shared-state HA (ES / Prometheus / Grafana / Alertmanager); write-path API for additional resource families (NAT, routing, DPI, zones); watch / streaming endpoints; broader eBPF program types beyond the four owned loaders (sk_msg, sk_lookup, lwt, etc.) |
...
| Production Ready | ❌ NO (estimated ~82-87%) | Sprint 31 narrowed the gap further; residual blockers are external-daemon and shared-state HA, mTLS controller-to-controller and external-daemon TLS, and write-path API breadth |

## Priority Next Steps

Sprint 31 (tickets 47-55) is fully merged. The next phase is **Sprint 32 (placeholder)**; detailed ticket definitions come in a separate planning session. Placeholder scope lives in `docs/design/implementation_backlog.md` §"Sprint 32 (placeholder): Post-Sprint-31 Production Hardening".

Candidate Sprint 32 workstreams (in rough priority order):

1. **mTLS for controller-to-controller calls** — Ticket 49 shipped server-only TLS on every owned listener except the API server; other listeners run `ClientAuth = NoClientCert`. A follow-up should flip them to `RequireAndVerifyClientCert` and define the cross-controller Subject-CN / SAN allowlist policy.
2. **External-daemon TLS** — FRR vtysh, Suricata socket, Kea control socket, Zeek Broker, and chronyc still speak plaintext on in-pod loopback / Unix paths; cross-host paths need a sidecar TLS terminator or daemon-native TLS.
3. **Prometheus rekey for `fos1-internal-ca`** — scrape configs in `manifests/base/monitoring/prometheus.yaml` still use plaintext fallbacks; targets that flip to HTTPS will fail closed under default trust until `tls_config.ca_file` is wired.
4. **External-daemon HA** — Ticket 47 covered controller-tier failover only. FRR / Suricata / Zeek / Kea singletons run as single-pod / single-process; per-daemon clustering (BFD for FRR, Kea HA hooks, parallel Suricata sensors) remains open.
5. **Shared-state HA (Elasticsearch / Prometheus / Grafana / Alertmanager)** — single-replica StatefulSets / Deployments hold persistent data. Multi-node clustering (ES cross-zone replication, Prometheus federation or Thanos) is a separate sprint.
6. **Write-path API for additional resource families** — Ticket 48 extended FilterPolicy to full CRUD. NAT, routing, DPI, zones, and threat feeds remain read-only or `kubectl`-only.
7. **Watch / streaming endpoints on the REST API** — chunked JSON-lines streaming was deferred from Ticket 48.
8. **Broader eBPF program types** — Ticket 51 added sockops + cgroup; sk_msg, sk_lookup, lwt, and other program types still return `ErrEBPFProgramTypeUnsupported` from `pkg/hardware/ebpf/program_manager.go`.
...
| **Internal TLS Baseline** | `pkg/security/certificates/`, `manifests/base/certificates/cluster-issuer-internal.yaml` | - | Complete (server-side) | Sprint 31 Ticket 49. Single `fos1-internal-ca` ClusterIssuer (CA-typed, chained from a 10y self-signed root) mints per-controller server certs at `/var/run/secrets/fos1.io/tls/`. `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` reload cert-manager renewals in place via fsnotify (no pod restart, no listener bounce). API server is mTLS-only (`RequireAndVerifyClientCert`); other owned listeners (NTP exporter, DPI metrics, correlator probes) serve TLS-only with `ClientAuth = NoClientCert`. `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew`. mTLS controller-to-controller, TLS for external daemons (FRR / Suricata / Kea / Zeek / chronyc), and Prometheus rekey for the chain remain Sprint 32 follow-ups. See `docs/design/internal-tls-secrets.md`. |
...
- **Inter-controller TLS baseline** — single `fos1-internal-ca` ClusterIssuer (CA-typed, chained from a 10y self-signed root) mints per-controller server certs at `/var/run/secrets/fos1.io/tls/`. `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` reload cert-manager renewals in place via fsnotify. API server is mTLS-only; other owned listeners (NTP exporter, DPI metrics, correlator probes) serve TLS-only with `ClientAuth = NoClientCert`. `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew` (Ticket 49). See `docs/design/internal-tls-secrets.md`.
...
### 1. mTLS Controller-To-Controller + External-Daemon TLS ⚠️ (Sprint 32 candidate)
...
**Still Missing:**
- Other owned listeners run `ClientAuth = NoClientCert`. A follow-up should flip them to `RequireAndVerifyClientCert` and define the cross-controller Subject-CN / SAN allowlist policy.
- Suricata's Unix socket, Zeek Broker, Kea's control socket, FRR's vtysh, and chronyc all live inside the same pod as their controller and speak plaintext on a loopback / Unix path. Cross-host paths are the Sprint 32 follow-up — likely a sidecar TLS terminator.
- Prometheus scrape configs in `manifests/base/monitoring/prometheus.yaml` still use plaintext fallbacks. Targets that flip to HTTPS will fail closed under default trust until `tls_config.ca_file` is wired to the `fos1-internal-ca` chain.
...
1. **mTLS Controller-To-Controller** - Ticket 49 shipped server-only TLS on every owned listener except the API server; other listeners run `ClientAuth = NoClientCert`. A follow-up should flip them to `RequireAndVerifyClientCert`.
2. **External-Daemon TLS** - FRR vtysh, Suricata socket, Kea control socket, Zeek Broker, and chronyc still speak plaintext on in-pod loopback / Unix paths. Cross-host paths need a sidecar TLS terminator or daemon-native TLS.
3. **Prometheus Rekey for `fos1-internal-ca`** - Scrape configs still use plaintext fallbacks; targets that flip to HTTPS will fail closed under default trust until `tls_config.ca_file` is wired.
4. **External-Daemon HA** - FRR / Suricata / Zeek / Kea singletons run as single-pod / single-process; per-daemon clustering (BFD for FRR, Kea HA hooks, parallel Suricata sensors) remains open.
5. **Shared-State HA (Elasticsearch / Prometheus / Grafana / Alertmanager)** - Single-replica StatefulSets / Deployments hold persistent data. Multi-node clustering (ES cross-zone replication, Prometheus federation or Thanos) is a separate sprint.
6. **Write-Path API for Additional Resource Families** - Ticket 48 extended FilterPolicy to full CRUD. NAT, routing, DPI, zones, and threat feeds remain read-only or `kubectl`-only.
7. **Watch / Streaming Endpoints on the REST API** - Chunked JSON-lines streaming was deferred from Ticket 48.
8. **Broader eBPF Program Types** - Ticket 51 added sockops + cgroup; sk_msg, sk_lookup, lwt, and other program types still return `ErrEBPFProgramTypeUnsupported`.
...
17. ~~No Inter-Controller TLS Baseline~~ - **Resolved (baseline):** Sprint 31 Ticket 49 ships the `fos1-internal-ca` ClusterIssuer + cert-manager rotation reload. mTLS controller-to-controller, external-daemon TLS, and Prometheus rekey remain Sprint 32 follow-ups.
...
23. **mTLS Controller-To-Controller + External-Daemon TLS** - Ticket 49 shipped server-side TLS only; mutual auth across the mesh and TLS for external daemons (FRR / Suricata / Kea / Zeek / chronyc) remain open.
...
- mTLS controller-to-controller, external-daemon TLS, and Prometheus rekey for the `fos1-internal-ca` chain are Sprint 32 candidates
...
⚠️ **Remaining Gaps:**
- ~82-87% production ready (up from ~75-80% pre-Sprint-31)
...
- mTLS controller-to-controller and external-daemon TLS still open (Ticket 49 shipped server-side TLS only on owned listeners other than the API server; the mTLS path covers only the API server).
- Prometheus rekey for the `fos1-internal-ca` chain still open (scrape configs use plaintext fallbacks).
...
**Verdict:** This has evolved from an architectural blueprint to a **functional beta system** with real daemon integrations (FRR, Suricata, Zeek, Kea, CoreDNS, AdGuard), real Cilium policy generation via FilterPolicy translation, working authentication providers (local/LDAP/OAuth), Kind-harness E2E proofs for the event correlator, Elasticsearch rollover, natural-traffic DPI, dashboard/alert PromQL validity, leader failover RTO ≤ 30s, and cert rotation across `cmctl renew`; real eBPF XDP + TC + sockops + cgroup compile/load on Linux; a CRUD-v1 REST API behind mTLS; a minimum-privilege RBAC baseline with CI enforcement; a four-hot-path performance baseline; URLhaus + MISP threat-intel; per-pod egress QoS via Cilium Bandwidth Manager plus VLAN-scoped TC shaping via the new `TrafficShaper` CRD; controller leader election with hot standby; and an internal TLS baseline via the `fos1-internal-ca` ClusterIssuer with cert-manager rotation. Sprint 31 closed the residual production-hardening blockers Sprint 30 had deferred.

**Recommendation:** The core routing, NAT, DNS, DHCP, IDS/IPS, DPI, filter-policy, eBPF, QoS, threat-intel, CRUD-v1 API, controller HA, and internal TLS pipelines are now functional and proven in the Kind harness and unit tests. Focus Sprint 32 on external-daemon and shared-state HA (largest single residual), mTLS controller-to-controller, external-daemon TLS, Prometheus rekey, write-path API for additional resource families, watch / streaming endpoints, and broader eBPF program types beyond the four owned loaders. See `docs/design/implementation_backlog.md` §"Sprint 32 (placeholder): Post-Sprint-31 Production Hardening" for the candidate scope.
