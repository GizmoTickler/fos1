...
// Manager implements the hardware.NICManager interface.
type Manager struct {
	interfaces     map[string]*hardware.NetworkInterface
...
func NewManager() (*Manager, error) {
	...
	return &Manager{
		interfaces:     make(map[string]*hardware.NetworkInterface),
...
func (m *Manager) ConfigureInterface(name string, config hardware.InterfaceConfig) error {
...
func (m *Manager) GetInterface(name string) (*hardware.NetworkInterface, error) {
...
func (m *Manager) ListInterfaces() ([]*hardware.NetworkInterface, error) {
	m.interfacesMu.RLock()
	defer m.interfacesMu.RUnlock()
	
	interfaces := make([]*hardware.NetworkInterface, 0, len(m.interfaces))
...
func (m *Manager) discoverInterfaces() error {
	...
	for _, link := range links {
		...
		iface := &hardware.NetworkInterface{
			Name: link.Attrs().Name,
			Type: hardware.InterfaceTypePhysical,
...
func (m *Manager) getOffloadFeatures(ifName string) (hardware.OffloadFeatures, error) {
