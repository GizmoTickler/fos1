...
// TestBGPHandler_ParseBGPSummaryJSON tests parsing of FRR "show bgp summary json" output
func TestBGPHandler_ParseBGPSummaryJSON(t *testing.T) {
	handler := &BGPHandler{
	...
	// Simulate FRR "show bgp summary json" output
	data := map[string]interface{}{
		"ipv4Unicast": map[string]interface{}{
			"routerId": "10.0.0.1",
			"as":       float64(65000),
			"peers": map[string]interface{}{
				"10.0.0.2": map[string]interface{}{
					"state":  "Established",
					"pfxRcd": float64(15),
					"pfxSnt": float64(10),
				},
				"10.0.0.3": map[string]interface{}{
					"state":  "Connect",
					"pfxRcd": float64(0),
					"pfxSnt": float64(0),
				},
		...
		"ipv6Unicast": map[string]interface{}{
			"routerId": "10.0.0.1",
			"as":       float64(65000),
			"peers": map[string]interface{}{
				"fd00::2": map[string]interface{}{
					"state":  "Established",
					"pfxRcd": float64(5),
					"pfxSnt": float64(3),
				},
	...
	err := handler.parseBGPSummaryJSON(data)
	if err != nil {
		t.Fatalf("parseBGPSummaryJSON failed: %v", err)
	}
	...
	// Check total prefixes received (15 + 0 + 5 = 20)
	if handler.status.PrefixesReceived != 20 {
		t.Fatalf("expected 20 total prefixes received, got %d", handler.status.PrefixesReceived)
	}

	// Verify individual neighbors
	...
	for _, n := range handler.status.Neighbors {
		...
		case "10.0.0.2":
			if n.State != "Established" {
				t.Errorf("expected 10.0.0.2 state Established, got %s", n.State)
			}
			if n.PrefixesReceived != 15 {
				t.Errorf("expected 10.0.0.2 prefixesReceived 15, got %d", n.PrefixesReceived)
			}
			if n.PrefixesSent != 10 {
				t.Errorf("expected 10.0.0.2 prefixesSent 10, got %d", n.PrefixesSent)
			}
		case "10.0.0.3":
			if n.State != "Connect" {
				t.Errorf("expected 10.0.0.3 state Connect, got %s", n.State)
			}
		case "fd00::2":
			if n.State != "Established" {
				t.Errorf("expected fd00::2 state Established, got %s", n.State)
			}
			if n.PrefixesReceived != 5 {
				t.Errorf("expected fd00::2 prefixesReceived 5, got %d", n.PrefixesReceived)
			}
		}
	}

	for _, addr := range []string{"10.0.0.2", "10.0.0.3", "fd00::2"} {
		if !found[addr] {
			t.Errorf("neighbor %s not found in status", addr)
		}
	}
}

// TestBGPHandler_ParseBGPSummaryJSON_EmptyPeers tests with no peers configured
func TestBGPHandler_ParseBGPSummaryJSON_EmptyPeers(t *testing.T) {
	handler := &BGPHandler{
		status: &routing.ProtocolStatus{
			Name:      "bgp",
			State:     "running",
...
// TestBGPHandler_ParseBGPSummaryJSON_NoPeersKey tests with missing peers key
func TestBGPHandler_ParseBGPSummaryJSON_NoPeersKey(t *testing.T) {
	handler := &BGPHandler{
		status: &routing.ProtocolStatus{
			Name:      "bgp",
			State:     "running",
	...
	err := handler.parseBGPSummaryJSON(data)
	if err != nil {
		t.Fatalf("parseBGPSummaryJSON should not fail on missing peers: %v", err)
	}
