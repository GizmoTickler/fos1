...
// --- FirewallController tests ---

func TestFirewallController_HandleCreateOrUpdate_Success(t *testing.T) {
	...
	obj := &unstructured.Unstructured{
		Object: map[string]interface{}{
			...
			"spec": map[string]interface{}{
				...
				"destinationSelector": map[string]interface{}{
					"matchLabels": map[string]interface{}{
						"zone": "wan",
					},
				},
	...
	err := controller.handleFirewallRuleCreateOrUpdate(obj)
	require.NoError(t, err)

	policy, exists := client.policies["fw-test-fw"]
	require.True(t, exists, "Cilium policy should be created")
	assert.Equal(t, "fw-test-fw", policy.Name)
	assert.Equal(t, "default", policy.Namespace)
	assert.Equal(t, "lan", policy.SourceLabels["zone"])
	assert.Equal(t, "wan", policy.DestinationLabels["zone"])
	require.Len(t, policy.Rules, 1)
	assert.Equal(t, "TCP", policy.Rules[0].Protocol)
	assert.Equal(t, []int{80, 443}, policy.Rules[0].Ports)
	assert.Equal(t, "allow", policy.Rules[0].Action)
}

func TestFirewallController_HandleCreateOrUpdate_MissingSpec(t *testing.T) {
	client := newTestCiliumClient()
	fakeClient := newControllerFakeDynamicClient()
...
func TestFirewallController_HandleCreateOrUpdate_CiliumError(t *testing.T) {
	client := newTestCiliumClient()
	...
	err := controller.handleFirewallRuleCreateOrUpdate(obj)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "error applying Cilium network policy")
}

func TestFirewallController_HandleDelete(t *testing.T) {
	client := newTestCiliumClient()
	...
	// Delete should succeed even for non-existent rules (placeholder implementation)
	err := controller.handleFirewallRuleDelete("default/test-fw")
	require.NoError(t, err)
}

func TestFirewallController_ConvertToCiliumRule_WithPortRanges(t *testing.T) {
	controller := &FirewallController{}
	...
	ciliumRule, err := controller.convertToCiliumRule(rule)
	require.NoError(t, err)
	assert.Equal(t, "TCP", ciliumRule.Protocol)
	assert.Equal(t, "deny", ciliumRule.Action)
	require.Len(t, ciliumRule.PortRanges, 1)
	assert.Equal(t, 8000, ciliumRule.PortRanges[0].First)
	assert.Equal(t, 9000, ciliumRule.PortRanges[0].Last)
}

func TestFirewallController_ConvertToCiliumRule_ICMP(t *testing.T) {
	controller := &FirewallController{}
	...
	ciliumRule, err := controller.convertToCiliumRule(rule)
	require.NoError(t, err)
	assert.Equal(t, "ICMP", ciliumRule.Protocol)
	require.NotNil(t, ciliumRule.ICMPType)
	assert.Equal(t, 8, *ciliumRule.ICMPType)
	require.NotNil(t, ciliumRule.ICMPCode)
	assert.Equal(t, 0, *ciliumRule.ICMPCode)
}

func TestFirewallController_ConvertToCiliumRule_MissingProtocol(t *testing.T) {
	controller := &FirewallController{}
	...
	_, err := controller.convertToCiliumRule(rule)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "error getting protocol")
}
...
func TestGetCiliumRuleNameForFirewallRule(t *testing.T) {
	name := GetCiliumRuleNameForFirewallRule("ns", "rule1")
	assert.Equal(t, "fw-ns-rule1", name)
}
...
// --- Firewall L7 rule tests ---

func TestFirewallController_ConvertToCiliumRule_WithHTTPRules(t *testing.T) {
	controller := &FirewallController{}
	...
	ciliumRule, err := controller.convertToCiliumRule(rule)
	require.NoError(t, err)
	require.Len(t, ciliumRule.L7Rules.HTTP, 1)
	assert.Equal(t, "GET", ciliumRule.L7Rules.HTTP[0].Method)
	assert.Equal(t, "/api/v1", ciliumRule.L7Rules.HTTP[0].Path)
	assert.Equal(t, "example.com", ciliumRule.L7Rules.HTTP[0].Host)
	assert.Equal(t, "Bearer token", ciliumRule.L7Rules.HTTP[0].Headers["Authorization"])
}

func TestFirewallController_ConvertToCiliumRule_WithDNSRules(t *testing.T) {
	controller := &FirewallController{}
	...
	ciliumRule, err := controller.convertToCiliumRule(rule)
	require.NoError(t, err)
	require.Len(t, ciliumRule.L7Rules.DNS, 1)
	assert.Equal(t, "*.example.com", ciliumRule.L7Rules.DNS[0].Pattern)
}
