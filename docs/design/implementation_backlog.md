...
- **mTLS for controller-to-controller calls** — Ticket 56 is in progress. The shared mutual-TLS loader, client helper, receiver Subject-CN allowlist middleware, and currently owned listener wiring have landed. Ticket 57 adds Prometheus client-cert scrape wiring for owned metrics, while external-daemon TLS remains Tickets 58-62.
- **External-daemon TLS (FRR vtysh, Suricata command socket, Kea control socket, Zeek Broker, chronyc)** — Tickets 58 and 59 now cover FRR vtysh and Suricata command traffic with repo-owned mTLS sidecars. Kea, Zeek, and chrony still speak plaintext on in-pod loopback / Unix paths. Cross-host paths need a sidecar TLS terminator or a daemon-native TLS configuration.
- **Prometheus rekey for `fos1-internal-ca`** — Ticket 57 wires the `prometheus-client-tls` client identity and dedicated HTTPS pod-SD jobs for the owned DPI/NTP metrics endpoints. Remaining operator work is CA replacement policy only: production overlays that swap the root issuer must provide the matching Prometheus CA bundle and client cert.
