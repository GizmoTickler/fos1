...
- **mTLS for controller-to-controller calls** — Ticket 56 now provides the shared mutual-TLS loader, client helper, receiver allowlist middleware, and in-code listener wiring for currently owned HTTP listeners. Live Prometheus scrape compatibility still depends on Ticket 57 because Prometheus must mount client certs and trust `fos1-internal-ca`.
