...
- Ticket 49: inter-controller TLS baseline. `fos1-internal-ca` ClusterIssuer (CA-typed, chained from a 10y self-signed root) mints per-controller server certs; `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` reloads cert-manager renewals in place via fsnotify. API server is mTLS-only (`RequireAndVerifyClientCert`); other listeners are TLS-only (`ClientAuth = NoClientCert`). `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew`. mTLS for controller-to-controller calls and TLS for external daemons (FRR / Suricata / Kea / Zeek) deferred to Sprint 32. See `docs/design/internal-tls-secrets.md`. Landed directly on main as `c60f906` (no merge commit).
...
See `docs/design/implementation_backlog.md` for full ticket definitions. The next phase is Sprint 32 (placeholder; scope finalization out of scope for Ticket 55). A placeholder with candidate scope lives in `docs/design/implementation_backlog.md` §"Sprint 32 (placeholder): Post-Sprint-31 Production Hardening".
...
- The next phase is Sprint 32 (placeholder scope in the backlog; detailed ticket definitions come in a separate planning session). Candidate focus areas are mTLS controller-to-controller calls, external-daemon TLS (FRR / Suricata / Kea / Zeek), Prometheus rekey for the `fos1-internal-ca` chain, external-daemon HA, shared-state HA (Elasticsearch / Prometheus / Grafana / Alertmanager), write-path API for additional resource families (NAT, routing, DPI, zones), watch / streaming endpoints, and broader eBPF program types beyond the four owned loaders (sk_msg, sk_lookup, etc.).
...
Sprint 31 is fully merged. The next phase is **Sprint 32 (placeholder)**; candidate scope lives in `docs/design/implementation_backlog.md` §"Sprint 32 (placeholder): Post-Sprint-31 Production Hardening". Detailed ticket definitions come in a separate planning session and are intentionally not duplicated here.
...
- **mTLS for controller-to-controller calls** — Ticket 49 shipped server-only TLS on every owned listener except the API server; other listeners run `ClientAuth = NoClientCert`. A follow-up should flip them to `RequireAndVerifyClientCert` and define the cross-controller Subject-CN / SAN allowlist policy.
...
- **Prometheus rekey for `fos1-internal-ca`** — scrape configs in `manifests/base/monitoring/prometheus.yaml` still use plaintext fallbacks; targets that flip to HTTPS will fail closed under default trust until `tls_config.ca_file` is wired.
