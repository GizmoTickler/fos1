...
- Sprint 31 closures (post-Sprint-30 production hardening, fully merged):
  ...
  - Ticket 49: inter-controller TLS baseline. `fos1-internal-ca` ClusterIssuer (CA-typed, chained from a 10y self-signed root) mints per-controller server certs; `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` reload cert-manager renewals in place via fsnotify (no pod restart, no listener bounce). API server stays mTLS-only (`RequireAndVerifyClientCert`); other owned listeners (NTP exporter, DPI metrics, correlator probes) serve TLS-only with `ClientAuth = NoClientCert`. `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew`. mTLS for controller-to-controller calls and TLS for external daemons (FRR / Suricata / Kea / Zeek) deferred to Sprint 32. Prometheus rekey for the chain is also a follow-up. See `docs/design/internal-tls-secrets.md`. Landed directly on main as `c60f906` (no merge commit).
...
- [x] Inter-Controller TLS Baseline (Sprint 31 Ticket 49; `fos1-internal-ca` ClusterIssuer chained from a 10y self-signed root mints per-controller server certs; `pkg/security/certificates.LoadTLSConfig` + `WatchAndReload` reload cert-manager renewals in place via fsnotify; API server is mTLS-only, other owned listeners are TLS-only; `scripts/ci/prove-cert-rotation.sh` asserts `/healthz` stays 200 across `cmctl renew`. mTLS controller-to-controller, TLS for external daemons, and Prometheus rekey for the chain remain Sprint 32 follow-ups.)
