...
- Sprint 32 active work:
  - Ticket 56: mTLS controller-to-controller mesh is in progress. `pkg/security/certificates.LoadMutualTLSConfig` now installs mounted cert material for both server and client auth, `NewMutualTLSHTTPClient` gives future controller clients the same trust anchor, and non-API owned listeners wrap handlers with deny-by-default Subject-CN allowlists. `scripts/ci/prove-mtls-mesh.sh` covers allowed cert success, no-cert handshake failure, and unknown-CN 403 behavior. Prometheus still needs Ticket 57 to mount client certs and trust `fos1-internal-ca` for live scrapes.
...
- [ ] mTLS Controller Mesh (Sprint 32 Ticket 56 in progress; shared `LoadMutualTLSConfig`, `NewMutualTLSHTTPClient`, and Subject-CN allowlist middleware landed for owned listeners. External-daemon TLS and Prometheus client-cert scrape wiring remain follow-ups.)
